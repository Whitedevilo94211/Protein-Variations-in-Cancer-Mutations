...
/**
 * Load CSV file from 'disease_data.csv'
 */
function loadCSV() {
  fetch('disease_data.csv')
    .then(response => {
      if (!response.ok) {
...
function searchGene() {
  ...
  const results = csvData.filter(row => {
    const protein_ID = row.Protein_ID ? row.Protein_ID.toLowerCase() : "";
    const mutation = row.Mutation ? row.Mutation.toLowerCase() : "";
    const structural_Impact = row.Structural_Impact ? row.Structural_Impact.toLowerCase() : "";
    const source_Database = row.Source_Database ? row.Source_Database.toLowerCase() : "";
    return (
      protein_ID.includes(searchTerm) ||
      mutation.includes(searchTerm) ||
      structural_Impact.includes(searchTerm) ||
      source_Database.includes(searchTerm)
    );
  });
